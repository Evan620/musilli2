...
export const GoogleSignIn: React.FC<GoogleSignInProps> = ({
  ...
  const [useGoogleButton, setUseGoogleButton] = React.useState(false);
    ...
    const initializeGoogle = () => {
      if (window.google && !isInitialized.current) {
        ...
        if (clientId && clientId !== "your_google_client_id_here.apps.googleusercontent.com") {
          ...
          // No valid Google Client ID, use fallback button
          setUseGoogleButton(false);
        }
      }
    };
  ...
  return (
    <div className={className}>
      {useGoogleButton ? (
        // Google's rendered button (only if properly configured)
